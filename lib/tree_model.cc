...
void TreeItem::track_folder(const TreeIndex &ti, const QString &dir,
                            Settings &settings)
{
  setData(TreeIndex::LOCAL_DIR, dir);
  settings.set(ti.id(), Settings::LOCAL_DIR, dir);
  settings.set(ti.id(), Settings::TRACKED, true);
  settings.sync();
}

bool TreeItem::has_untargeted()
{
  bool has = isChecked() && !has_local_dir();
  QList<TreeItem *> children = childrenItems();
  for (auto child : children)
    has |= child->has_untargeted();
  return has;
}
                         ...
                         std::filesystem::path *cwd, std::vector<Folder> *list)
{
  ...
  // one occurrence of a checked folder
  if (item->isChecked()) {
    new_base = local_dir;
  }
...
QVariant TreeModel::data(const QModelIndex &index, int role) const
{
  ...
  if (role == Qt::DisplayRole || role == Qt::EditRole) {
    ...
    if (index.column() == TreeIndex::LOCAL_DIR) {
      ...
      if (str.isEmpty() && item->isChecked()) return TreeIndex::UNLINKED;
      return data.toString().replace(QDir::homePath(), "~");
    }
    return data;
  }

  if (role == Qt::CheckStateRole) {
    if (!index.parent().isValid() || index.column() != TreeIndex::REMOTE_DIR)
      return QVariant();
                        ...
                        int role)
{
  ...
  if (role == Qt::CheckStateRole) {
    ...
    if (item->isChecked()) {
      item->on_all_parents([&](TreeItem &t) {
        t.setChecked(false);
        settings->set(t.id(), Settings::TRACKED, false);
        const QModelIndex i = this->indexFromItem(&t);
        emit dataChanged(i, i);
      });
      item->on_all_children([&](TreeItem &t) {
        t.setChecked(false);
        settings->set(t.id(), Settings::TRACKED, false);
        const QModelIndex i = this->indexFromItem(&t);
        emit dataChanged(i, i);
